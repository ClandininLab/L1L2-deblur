% compute_meanResp_err_moveAvg_FFFoG.m
%
...
%
% Updated: 1/17/17 HHY - change to deal with stimDat appropriately, after
%  change in loadROIData.m
% Updated: 5/18/17 HHY - change how frames around epoch transitions are
%  handled (spills over into previous or next epoch, instead of clipping).
%  Add time stamp output.
% Updated: 6/21/17 HHY - when length of stimulus doesn't divide evenly into
%  given number of frames, round down, not up (see line 55)
%
function [meanResp, stdErr, t] = ...
    compute_meanResp_err_moveAvg_FFFoG(roi, seqDurs, ...
    BIN_SHIFT, BIN_WIDTH, FRAME_RATE)    
    
    % after change in loadROIData to deal with stimDat differently
%     nFlashEpochs = length(roi.stimDat.obj.FlashContrast);
    nFlashEpochs = length(roi.stimDat.FlashContrast);
...
%         framesPerEpoch = ceil(seqDurs(i) * FRAME_RATE);
        ...
        rcStimInd = roi.pStimDat.rcStimInd;
        g2fTimes = roi.pStimDat.stimEpochStartTimes(rcStimInd == i);
