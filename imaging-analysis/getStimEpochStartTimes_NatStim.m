...
function [epochStartTimes, lightStartTimes, darkStartTimes, order] = ...
    ...
    ldIndex = find(pdData > thresh); 
    % dark indices
    ddIndex = find(pdData <= thresh);
    
    % higher the threshold means longer buffer (seconds)
%     buffer = (1/NIDAQScanRate) * 2; 
%     interFrameDist = 0.003 *  + buffer; % distance between frames + a buffer (seconds)
%     patternLength = IFI/3; % 3 is the number of patterns in a frame

    % dark to light transitions
    % number of NIDAQ scans b/w each light index
    numScansLightInd = diff(ldIndex);
    % find when number of scans > 1 (there's a gap) - transitions, as
    % indicies into ldIndex
    d2LTransInd = find(numScansLightInd > 1) + 1;
    % dark to light transitions, as times; preceding 1 is for first
    % transition
    lightStartTimes = pdTime(ldIndex([1; d2LTransInd]));
    
    % light to dark transitions
    numScansDarkInd = diff(ddIndex);
    % find when number of scans > 1
    l2DTransInd = find(numScansDarkInd > 1) + 1;
    % light to dark transitions, as times
    darkStartTimes = pdTime(ddIndex(l2DTransInd));
