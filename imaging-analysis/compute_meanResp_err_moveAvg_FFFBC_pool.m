% compute_meanResp_err_moveAvg_FFFBC_pool.m
%
...
% Handles multiple trials for that ROI
% 
% INPUT:
...
function [slidingMean, slidingStdErr, sigPInd, meanDark, meanLight, ...
    ...
    compute_meanResp_err_moveAvg_FFFBC_pool(roi, ...
    ...
    % loop over all trials for this roi
    for r = 1:length(roi)

        % find stimulus transition times
        % dark to light
        dtlTimes = roi(r).pStimDat.lightStartTimes;
        % light to dark
        ltdTimes = roi(r).pStimDat.darkStartTimes;
    
        % do not count imaging frames before first dark to light transition
        startImgFrame = find((roi(r).imFrameStartTimes - dtlTimes(1))>0,...
            1,'first');
        ...
        for k = startImgFrame:length(roi(r).imFrameStartTimes)
            % current frame time
            ft = roi(r).imFrameStartTimes(k); 
            % time difference between stimulus transitions and current
            %  frame time
            tDiff = ft - roi(r).pStimDat.stimEpochStartTimes;
            ...
            if roi(r).pStimDat.rcStim(ind) == 1 
                ...
                if (minTimeDiff <= cycleDur/2) 
                    ...
                    lightDFF = [lightDFF; roi(r).dFF(k)];
                end 
            else 
                % when stimulus presented slightly longer than expected,
                %  ignore that img frame
                if (whichBin <= framesPerEpoch)
                    darkVals{whichBin} = [darkVals{whichBin}; roi(r).dFF(k)];
                end
