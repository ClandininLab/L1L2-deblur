% plot_shortFlashTemporalFrequency_compareGenotypes_v2.m
%
...
dataPath = '/Users/hyang/Documents/New Imaging Analysis/AnalyzedData/170927';
cd(dataPath)
...
for i = 1:size(pairedEpochs, 1) % number of flash lengths
    for j = 1:size(pairedEpochs, 2) % number of contrast values per flash length
        figure; 
        tfMag1 = zeros(length(respROIMat1), length(respROIMat1(1).tfMag{pairedEpochs(i,j)}));
        flyID1 = zeros(1, length(respROIMat1));
        for n = 1:length(respROIMat1)
            tfMag1(n, :) = respROIMat1(n).tfMag{pairedEpochs(i,j)};
            flyID1(n) = respROIMat1(n).flyID;
        end 
        m1 = mean(tfMag1,1);
        e1 = std(tfMag1,[],1)./sqrt(size(tfMag1,1));
        ...
        tfMag2 = zeros(length(respROIMat2), length(respROIMat2(1).tfMag{pairedEpochs(i,j)}));
        flyID2 = zeros(1, length(respROIMat2));
        for n = 1:length(respROIMat2)
            tfMag2(n, :) = respROIMat2(n).tfMag{pairedEpochs(i,j)};
            flyID2(n) = respROIMat2(n).flyID;
        end 
        m2 = mean(tfMag2,1);
        e2 = std(tfMag2,[],1)./sqrt(size(tfMag2,1));
        ...
        tfMag3 = zeros(length(respROIMat3), length(respROIMat3(1).tfMag{pairedEpochs(i,j)}));
        flyID3 = zeros(1, length(respROIMat3));
        for n = 1:length(respROIMat3)
            tfMag3(n, :) = respROIMat3(n).tfMag{pairedEpochs(i,j)};
            flyID3(n) = respROIMat3(n).flyID;
        end         
        m3 = mean(tfMag3,1);
        e3 = std(tfMag3,[],1)./sqrt(size(tfMag3,1));        
        ...
        tfMag4 = zeros(length(respROIMat4), length(respROIMat4(1).tfMag{pairedEpochs(i,j)}));
        flyID4 = zeros(1, length(respROIMat4));
        for n = 1:length(respROIMat4)
            tfMag4(n, :) = respROIMat4(n).tfMag{pairedEpochs(i,j)};
            flyID4(n) = respROIMat4(n).flyID;
        end         
        m4 = mean(tfMag4,1);
        e4 = std(tfMag4,[],1)./sqrt(size(tfMag4,1));        
        ...
        legend([h1 h2 h3 h4], ...
            [titleStr1 ', N cells = ' num2str(size(tfMag1,1)) ...
        ', N flies = ' num2str(length(unique(flyID1)))],...
        [titleStr2 ', N cells = ' num2str(size(tfMag2,1)) ...
        ', N flies = ' num2str(length(unique(flyID2)))],...
        [titleStr3 ', N cells = ' num2str(size(tfMag3,1)) ...
        ', N flies = ' num2str(length(unique(flyID3)))],...
        [titleStr4 ', N cells = ' num2str(size(tfMag4,1)) ...
