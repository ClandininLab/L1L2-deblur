% L1L2paper_impulseResponseModeling_v2.m
%
% Generate model impulse responses for extended data figure 4 for L1/L2
%  paper
%
% Updated: 8/17/18
%
...
%% generate contrast step - 0.25 s dark off of gray

flashParams.contrastStepDur = 0.25; % in sec
flashFrames = ceil(flashParams.contrastStepDur/ifi);
flashParams.contrastStepGrayDur = 0.25; % in sec
grayFrames = ceil(flashParams.contrastStepGrayDur/ifi);

flashParams.fullContrastStepStim = [zeros(1,grayFrames) ...
    -1*ones(1,flashFrames) zeros(1,grayFrames)];


%% convolve filter with contrast step

for n = 1:length(pFilters)
    % response
    pFilters(n).filtContrastStepOut = conv(...
        flashParams.fullContrastStepStim,pFilters(n).filter);
    
    % half-wave rectified response
    pFilters(n).rectContrastStepOut = pFilters(n).filtContrastStepOut;
    pFilters(n).rectContrastStepOut(...
        pFilters(n).rectContrastStepOut < 0) = 0;
end


% time points of response to repeated flash stimulus
tContrastStep = 0:(length(pFilters(1).filtContrastStepOut)-1);
tContrastStep = tContrastStep * ifi;


%% plot responses to step
...
for n = length(pFilters):-1:1
    plot(tContrastStep,pFilters(n).filtContrastStepOut,'color',...
        pFilters(n).color,'linewidth',1.5);   
end

legend(fliplr(legends),'location','northeast', 'AutoUpdate', 'off');
xlabel('Time (sec)');
ylabel('Response Amplitude');
title('Modeled Responses to Contrast Step Stimulus');
xlim([0.25 0.75]);
ylim([-5 5]);
line([0 1],[0 0],'color',[0 0 0]); % x-axis line
line([0.25 0.25], [-10 5], 'color', [0 0 0]);
line([0.5 0.5], [-10 5], 'color', [0 0 0]);
