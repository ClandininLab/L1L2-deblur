...
% current values (PWM) set on the LightCrafter and not stimuli
%
% INPUT:
...
function out = matchROIsAcrossLCCurrentVal(tsInds, refStimCode, refPWM, metaDat)
    ...
    refInd = find(refPWM==metaDat.pwm(tsInds).*strcmpi(refStimCode, metaDat.stimcode(tsInds)));
