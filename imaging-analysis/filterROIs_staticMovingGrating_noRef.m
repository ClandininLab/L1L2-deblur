% filterROIs_staticMovingGrating_noRef.m
%
% Function to obtain stimulus-locked average responses and error per ROI
%  for presentations of static and moving bar stimulus. No reference 
%  stimulus
%
% INPUT:
...
function [roiMat, iResp, iInv, layer] = ...
    filterROIs_staticMovingGrating_noRef(roiMat, FRAME_RATE, binWidthMult)
...
%             elseif (s == fffgColumn)
                ...
                grayDuration = cell2mat(roiMat(r,s).stimDat.GrayDuration);
                staticDuration = cell2mat(roiMat(r,s).stimDat.StaticDuration);
                movingDuration = cell2mat(roiMat(r,s).stimDat.MovingDuration);
                epochDuration = grayDuration + staticDuration + ...
                    movingDuration;
                
                % compute mean and error
                [meanResp, stdErr] = ...
                    compute_meanResp_err_moveAvg_staticMovingGrating(...
                    roiMat(r,s),...
...
%             'r*');
        ...
        for j = 1:numEpochs
            ...
            % line marking start of static grating period
            line([grayDuration(j) grayDuration(j)], yScale, 'color',...
                [0 0 0], 'LineStyle', '--');
            
            % line marking start of moving period
            line([grayDuration(j)+staticDuration(j), ...
                grayDuration(j)+staticDuration(j)], yScale, 'color',...
                [0 0 0], 'LineStyle', '--');
        end
