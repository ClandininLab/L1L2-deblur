% SearchStimulusFlash_RGB.m
%
% Version of SearchStimulusFlash that works with lightcrafter
...
%  SearchStimulusFlash and not 3X the resolution, though this
...
%
% Updates:
%   8/9/19 - HHY - created file
%   8/30/19 - HHY - fixed bug for saving rawStim
% last update: 8/30/19 HHY
% 
classdef SearchStimulusFlash_RGB < Stimulus 
    properties 
        Out
    ...
    methods
        % Constructor
        function obj = SearchStimulusFlash_RGB(txtFile)
...
%           % vector to save actual stimulus played.
            ...
            for frame = 1:totalFrames
                ...
                % define background and center colors as RGB
                bkgdRGB = convertInt2RGB(...
                    repmat(obj.BackgroundColor{currEpoch},1,3));
                centerRGB = convertInt2RGB(...
                    repmat(obj.CenterColor{currEpoch},1,3));
                
                % Draw the background onto the screen area
                Screen('FillRect', window, bkgdRGB, obj.screenDim);
                % Draw the center as convex polygon defined by verticies
                Screen('FillPoly', window, centerRGB, verticies, 1);
...
%                     obj.pdDim);
                ...
                if obj.CenterColor{currEpoch} ~= ...
                    ...
                    Screen('FillRect', window,...
                        convertInt2RGB(ones(1,3)),obj.pdDim);
                else
                    Screen('FillRect', window, ...
                        convertInt2RGB(zeros(1,3)),obj.pdDim);
                end
                ...
                % record stimulus info, background and center color; with
                %  RGB conversion in case rounding difference
                bkgdInt = convertRGB2Int(bkgdRGB);
                centerInt = convertRGB2Int(centerRGB);
                
                % all 3 elements of bkgdRGB and center RGB are the same,
                %  take only the first one
                rawStim(frame,:) = [bkgdInt(1), centerInt(1)];    
