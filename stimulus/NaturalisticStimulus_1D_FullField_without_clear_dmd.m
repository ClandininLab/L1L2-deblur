...
%   7/20/22 - HHY - stimulus reconstruction from photodiode now without DMD
%   clearing (old code commented out)
% 
...
%             end 
        ...
        function [rcStim, rcStimInd] = reconstructStim(obj, ...
            ...
            % FOR RECONSTRUCTING STIMULUS, WITH DMD CLEARING %
...
%             end
            ...
            % FOR RECONSTRUCTING STIMULUS WITHOUT DMD CLEARING 7/20/22 %
            % epoch start is when duration of on is 4 patterns long
            %  followed by 2 patterns long, continuous
            % assumes 3 patterns/frame

            % initialize array for epoch starts
            epochStartTimes = [];
            
            % get duration of each on flash
            lightOnDur = darkStartTimes - lightStartTimes;

            % get indices of light on durations that are 3.5 to 4.5
            % patterns long
            longDurInd = find((lightOnDur > (obj.Out.stimIFI * (3.5/3))) & ...
                (lightOnDur < (obj.Out.stimIFI * (4.5/3))));

            % get indices of light durations that are 1.5 to 2.5 patterns
            %  long
            medDurInd = find((lightOnDur > (obj.Out.stimIFI * (1.5/3))) & ...
                (lightOnDur < (obj.Out.stimIFI * (2.5/3))));

            % for each longDurInd, check that subsequent flash is med dur
            % if yes, that is an epoch start
            for i = 1:length(longDurInd)
                % if subsequent flash is med dur
                if (any(longDurInd(i) + 1 == medDurInd))
                    % add to epoch start array
                    epochStartTimes = [epochStartTimes; ...
                        lightStartTimes(longDurInd(i))];
                end
            end
